...
namespace pkmn_ntr.Sub_forms
{
    public partial class f2_Text : Form
    {
        ...
        private readonly bool bigendian = new[] { GameVersion.COLO, GameVersion.XD, GameVersion.BATREV, }.Contains(Program.gCmdWindow.SAV.Version);
        ...
        {
            ushort[] chars = getChars(Program.gCmdWindow.SAV.Generation);
            if (chars.Length == 0)
                return;
        ...
        private void addTrashEditing(int length)
        {
            ...
            NUD_Generation.Value = Program.gCmdWindow.SAV.Generation;
            int charct = length;
            int bytesperchar = bigendian || Program.gCmdWindow.SAV.Generation > 3 ? 2 : 1;
        ...
        private void updateNUD(object sender, EventArgs e)
        {
            ...
            string str = PKX.getString(Raw, Program.gCmdWindow.SAV.Generation, Program.gCmdWindow.SAV.Japanese, bigendian, Raw.Length);
            TB_Text.Text = str;
            editing = false;
        }
        private void updateString(object sender, EventArgs e)
        {
            ...
            editing = true;
            // build bytes
            byte[] data = PKX.setString(TB_Text.Text, Program.gCmdWindow.SAV.Generation, Program.gCmdWindow.SAV.Japanese, bigendian, Raw.Length, Program.gCmdWindow.SAV.Language);
        ...
        private void B_ApplyTrash_Click(object sender, EventArgs e)
        {
            ...
            byte[] current = PKX.setString(TB_Text.Text, Program.gCmdWindow.SAV.Generation, Program.gCmdWindow.SAV.Japanese, bigendian, Raw.Length, Program.gCmdWindow.SAV.Language);
            byte[] data = PKX.setString(species, Program.gCmdWindow.SAV.Generation, Program.gCmdWindow.SAV.Japanese, bigendian, Raw.Length, Program.gCmdWindow.SAV.Language);
